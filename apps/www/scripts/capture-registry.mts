...
async function captureScreenshots() {
  ...
  const blocks = blockIds.filter((block) => {
    ...
    const lightPath = path.join(
      REGISTRY_PATH,
      `styles/ftech/${block}-light.png`
    )
    const darkPath = path.join(
      REGISTRY_PATH,
      `styles/ftech/${block}-dark.png`
    )
  ...
  for (const block of blocks) {
    const pageUrl = `http://localhost:3333/view/styles/ftech/${block}`
    ...
    for (const theme of ["light", "dark"]) {
      const screenshotPath = path.join(
        REGISTRY_PATH,
        `styles/ftech/${block}${theme === "dark" ? "-dark" : "-light"}.png`
      )
